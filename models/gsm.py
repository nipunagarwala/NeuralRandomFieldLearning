...
class GSM(Model):
    ...
    def __init__(
        ...
        Model.__init__(
            self, n_dim, n_chan, n_out,
            n_superbatch, opt_alg, opt_params)
    ...
    def create_model(self, X, Y, n_dim, n_out, n_chan=1):
        ...
        l_q_hid1 = lasagne.layers.DenseLayer(
            l_q_in, num_units=512,
            nonlinearity=hid_nl)
        l_q_hid2 = lasagne.layers.DenseLayer(
            l_q_hid1, num_units=256,
            nonlinearity=hid_nl)
        l_q_mu = lasagne.layers.DenseLayer(
            l_q_hid2, num_units=n_class*n_cat,
            nonlinearity=None)
        ...
        l_p_hid1 = lasagne.layers.DenseLayer(
            l_q_sample, num_units=256,
            nonlinearity=hid_nl)
        l_p_hid2 = lasagne.layers.DenseLayer(
            l_p_hid1, num_units=512,
            nonlinearity=hid_nl)
        l_p_mu = lasagne.layers.DenseLayer(
            l_q_hid2, num_units=n_out,
            nonlinearity=None)
